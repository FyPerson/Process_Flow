...
读完 codex 报告后，**必须**用以下双层结构汇报给用户，而不是直接转述 codex 的技术语言：

**第一层：非技术总结（最先说，2-4 句话）**
- 这次改动安全吗？（会不会影响现有用户的数据 / 功能 / 体验）
- 作为产品负责人需要知道的关键取舍是什么？
- 用户接下来要拍板什么？

**第二层：技术细节（接在总结后面，给愿意细看的用户）**
- codex 必修项 / 建议项原文要点
- 涉及的文件路径
- 实施工作量评估

**为什么这样汇报**：codex 的技术报告对工程上下文完整、对产品决策不友好。
用户是产品负责人 + 业务流程图工具的拥有者，需要知道的是"我点了'继续'之后会发生什么、风险在哪、用户体验是变好还是变差"，而不是"useStore 比 getNode 更响应式"。
如果只转述技术语言，用户的拍板会变成盲签。

**反例**（不要这样）：
> codex 指出 4 必修：(1) FlowCanvas useEdgesState 不响应外部 props 变化…(2) hasDeprecatedEndpoint 污染 storage…

**正例**（这样写）：
> **总结**：这次改动整体方向对，但有 1 个会让用户感到困惑的体验问题（标废弃后边不会立刻变淡，要刷新页面才生效）+ 1 个会让画布数据多出脏字段的小漏洞。两个都能修，不影响数据安全，但建议修完再发版。
>
> **细节**：codex 指出 4 必修…
