...
### 11. ❌ Hook 里调 ssh 报 "Could not resolve hostname X: Name or service not known"（阶段 0 部署切换发现）

post-receive hook 从 git push 触发时继承 git bash 的 PATH，`Get-Command ssh` 会找到 `C:\Program Files\Git\usr\bin\ssh.exe`（Cygwin 风格）。这个 ssh 对 Windows cmd 风格的双引号转义处理不一致，会把含空格的远端命令字符串拆成多个 argv 然后报 "Could not resolve hostname"（错把命令当 hostname）。

✅ 修复：在 [server-hooks/post-receive.ps1](server-hooks/post-receive.ps1) 里**显式用 Windows OpenSSH 绝对路径**：
```powershell
$sshExe = 'C:\Windows\System32\OpenSSH\ssh.exe'
$output = cmd /c "`"$sshExe`" $SshTarget `"$Command`" 2>&1"
```
不要 `Get-Command ssh` 让 PATH 决定，也不要做 `Test-Path -LiteralPath $sshExe` 检测——hook 在 Cygwin 派生的 PowerShell 里 Test-Path 对 Windows 系统目录可能行为异常（曾报"LiteralPath 空值"）。直接用绝对路径调 cmd，让 cmd 处理路径解析。
