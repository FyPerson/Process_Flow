...
### 12. 🔴 Hook 里 ssh 调用在 hook 派生 PowerShell 上下文里"空跑"（阶段 1 P1A-C 部署造成真事故）

**现象**：post-receive.ps1 用 `& $sshExe $SshTarget $Command 2>&1` 调用本应在 server 跑 `npm install` / `pm2 restart`，hook stdout 显示步骤完成（绿字），但 server 端实际**未执行**——pid 不变、↺ 不增、新依赖未装。
...
🔴 **真生产事故记录**：
- 阶段 1 P1A-C commit `104a76c` 加了 `dotenv` 依赖。Hook 跑了 git fetch+reset OK，但 npm install 静默失败 → PM2 restart 后 server 启动时 `Cannot find package 'dotenv'` → 进程崩溃循环（↺ 14）→ HTTP 504。手动 `ssh + npm install + pm2 restart` 才恢复。
- 阶段 2 P2H 期间（commit `2e0c7fa`）改了 `server/middleware/security.ts`。Hook 报告"PM2 restarted ✓"，但 `pm2 list` 显示 **uptime 5h** —— **进程整整 5 小时没重启过**。期间 7 个 P2H commit 全没在生产生效（用户访问还是旧代码）。手动 `pm2 restart business-flow` 后 uptime=0s 才真激活。**hook 报告全是假绿字**。

✅ **强制规则**（每次 push 后必看 commit 内容）：
- 如果 commit 改了 `package.json`（依赖变化）：push 后**必须**手动 `ssh administrator@172.16.0.138 'cd /d E:\business-flow && npm install --no-audit --no-fund'`
- 如果 commit 改了 `src/`（前端代码）：push 后**必须**手动 ssh 跑 `npm run build`
- 如果 commit 改了 `server/` / `package.json` / `ecosystem.config.cjs`（后端运行时）：push 后**必须**手动 `pm2 restart business-flow --update-env`
- 任何代码变更后用步骤 9.4 的 `/api/health` 验证 server 真起来了（mode=production + dbWritable=true）
- 🔴 **每次 push 后必须**用 `ssh administrator@172.16.0.138 'pm2 list'` 看 `uptime` 列：如果不是 `0s` 或 `<1m`，说明 hook 又静默失败了，进程还在跑老代码，**必须手动 restart**。仅看 hook 输出 / health 200 不够 —— health 是旧进程也能返。
- **建议把这三步合并成一行别名**：
  ```bash
...
- 选项 3：把 hook 从 post-receive 改到客户端 deploy script（与 push 同步执行 ssh 命令）—— 违背"push 即部署"，但最简单可靠
- **优先级 P2 → P1**：阶段 2 P2H 期间又踩了一次（这次 5 小时假重启）。已验证：**仅靠 hook 输出 + /api/health 都不够，必须 ssh + pm2 list 看 uptime**。阶段 3 实施前**必须**先解决底层 hook，否则每次部署都得手动 ssh，且容易因为忘记验 uptime 被假绿字骗
