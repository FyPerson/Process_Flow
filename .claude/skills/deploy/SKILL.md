...
🔴 **真生产事故记录**：阶段 1 P1A-C commit `104a76c` 加了 `dotenv` 依赖。Hook 跑了 git fetch+reset OK，但 npm install 静默失败 → PM2 restart 后 server 启动时 `Cannot find package 'dotenv'` → 进程崩溃循环（↺ 14）→ HTTP 504。手动 `ssh + npm install + pm2 restart` 才恢复。
...
- **优先级 P2**：阶段 2 / 阶段 3 任何会引入新依赖（zod / better-sqlite3 升级 / 加 dompurify 等）的部署都会再次踩；建议阶段 2 实施前先解决
