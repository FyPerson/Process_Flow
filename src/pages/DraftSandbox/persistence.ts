...
//   - version 字段防未来 schema 升级：v1 不匹配 → console warn + fallback 空画布
//   - 容错：localStorage 配额满 / 浏览器禁用 / JSON.parse 失败 → 不抛错，继续运行
...
/**
 * 写入 localStorage。失败时 console warn 不抛错（caller 继续工作）
 * 失败场景：localStorage 配额满 / 浏览器禁用 / JSON.stringify 失败（循环引用）
 */
export function saveDraft(nodes: Node<DraftNodeData>[], edges: Edge[]): void {
  try {
